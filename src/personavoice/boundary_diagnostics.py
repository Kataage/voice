...
def _normalize_text(text: str, *, remove_whitespace: bool = False) -> str:
    """Normalize ASR text without adding a v0.4 evaluation dependency."""
...
def _edit_distance(reference: str | tuple[str, ...], hypothesis: str | tuple[str, ...]) -> int:
    shorter, longer = (
        (reference, hypothesis) if len(reference) <= len(hypothesis) else (hypothesis, reference)
    )
    previous = list(range(len(shorter) + 1))
    for long_index, long_item in enumerate(longer, start=1):
        current = [long_index]
        for short_index, short_item in enumerate(shorter, start=1):
            current.append(
                min(
                    current[-1] + 1,
                    previous[short_index] + 1,
                    previous[short_index - 1] + (long_item != short_item),
                )
            )
        previous = current
    return previous[-1]
...
def _tokenize_words(text: str) -> tuple[str, ...]:
...
def _character_error_rate(reference: str, hypothesis: str) -> float:
    normalized_reference = _normalize_text(reference, remove_whitespace=True)
    normalized_hypothesis = _normalize_text(hypothesis, remove_whitespace=True)
...
def _state_provenance(paths: PersonaPaths) -> dict[str, Any]:
    """Capture existing stage identities without changing or revalidating them."""
    ...
    for name in ("prepare", "train"):
        ...
        if isinstance(stage, dict):
            output[f"{name}_fingerprint"] = stage.get("fingerprint")
            output[f"{name}_status"] = stage.get("status")
    return output
...
def _final_token_evidence(expected: str, hypothesis: str) -> dict[str, Any]:
    expected_normalized = _normalize_text(expected, remove_whitespace=True)
    hypothesis_normalized = _normalize_text(hypothesis, remove_whitespace=True)
    expected_words = _tokenize_words(expected)
    expected_token = expected_words[-1] if expected_words else ""
    final_token_preserved = bool(
        expected_token
        and hypothesis_normalized.endswith(
            _normalize_text(expected_token, remove_whitespace=True)
        )
    )
...
def _record_output(
    *,
    ...
    logs = "\n".join(
        value
        for value in (str(metadata.get("stdout") or ""), str(metadata.get("stderr") or ""))
        if value
    )
    record: dict[str, Any] = {
        "status": "ok",
        ...
        "duration": _parse_duration_log(logs),
        "final_loss_evidence": _parse_final_loss_evidence(logs),
    ...
    if asr is not None:
        ...
        cer = _character_error_rate(expected_text, transcript)
        wer = _word_error_rate(expected_text, transcript)
...
def run_boundary_diagnostics(
    ...
    *,
    ...
    report = {
        ...
        "persona": cfg.name,
        "issue": 34,
        "contract": "issue-33-boundary-diagnostics",
        "provenance": {
            "irodori_source_revision": IRODORI_SOURCE_REVISION,
            "diagnostic_schema": BOUNDARY_DIAGNOSTIC_SCHEMA,
            "runtime_contract": "irodori-boundary-inference-v1-v03",
            "stage_snapshot": _state_provenance(paths),
        },
        "policy": {
            "configured_duration_scale": cfg.inference.duration_scale,
            "configured_trim_tail": cfg.inference.trim_tail,
            "configured_tail_window_size": cfg.inference.tail_window_size,
            "configured_tail_std_threshold": cfg.inference.tail_std_threshold,
            "configured_tail_mean_threshold": cfg.inference.tail_mean_threshold,
            "inference_only": True,
