...
_BACKENDS = {
    ...
    "qwen": GENERAL_QWEN,
    "qwen3-asr-1.7b": GENERAL_QWEN,
    "qwen/qwen3-asr-1.7b": GENERAL_QWEN,
    "domain-qwen": DOMAIN_QWEN,
    "qwen3-asr-1.7b-ja-anime-galgame-hf": DOMAIN_QWEN,
...
def active_lineage_id(paths: PersonaPaths) -> str | None:
    ...
    try:
        ...
        return None
    selected = value.get("active_lineage_id") if isinstance(value, dict) else None
    return selected if isinstance(selected, str) and _LINEAGE_ID_RE.fullmatch(selected) else None


def effective_paths(paths: PersonaPaths) -> PersonaPaths:
    """Resolve the explicit active generation for runtime consumers."""

    if paths.lineage_id is not None:
        return paths
    selected = active_lineage_id(paths)
    return paths.for_lineage(selected) if selected is not None else paths
...
def _safe_candidate_file(candidate: PersonaPaths, relative: Any) -> Path:
    if not isinstance(relative, str) or not relative or Path(relative).is_absolute():
        raise RuntimeError("Generation artifact paths must be relative to the candidate lineage")
    target = (candidate.lineage_root / relative).resolve()
    try:
        target.relative_to(candidate.lineage_root.resolve())
    except ValueError as exc:
        raise RuntimeError("Generation artifact escapes its candidate lineage") from exc
    return target
...
def activate_generation(
    ...
    *,
    ...
    manifest_path = candidate.lineage_root / "generation.json"
    try:
    ...
    generation_fingerprint = _verify_generation_manifest(
        candidate,
        record,
        manifest,
    ...
    with stage_lock(paths.root, "activate"):
        ...
        if previous is not None:
            ...
            previous_digest = hashlib.sha256(_canonical(previous).encode("utf-8")).hexdigest()[:8]
            atomic_write_json(
