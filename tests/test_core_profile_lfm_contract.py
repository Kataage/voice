...
from personavoice.state import _jsonl_contract
from personavoice.training import (
    _invalidate_lfm_artifacts,
    _lfm_fingerprint,
    _non_lfm_fingerprint,
)
...
def test_profile_edit_does_not_change_training_scopes(tmp_path: Path):
    paths = init_persona(tmp_path, "alice", authorized=True)
    cfg = PersonaConfig.load(paths.config)
    before = (_non_lfm_fingerprint(paths, cfg), _lfm_fingerprint(paths, cfg))
    _profile().save(paths.core_profile)
    after = (_non_lfm_fingerprint(paths, cfg), _lfm_fingerprint(paths, cfg))
    assert after == before


def test_lfm_export_change_has_lfm_only_invalidation_scope(tmp_path: Path):
    paths = init_persona(tmp_path, "alice", authorized=True)
    cfg = PersonaConfig.load(paths.config)
    dataset = paths.dataset / "lfm_train.jsonl"
    dataset.write_text('{"legacy":true}\n', encoding="utf-8")
    before_non_lfm = _non_lfm_fingerprint(paths, cfg)
    before_lfm = _lfm_fingerprint(paths, cfg)
    dataset.write_text('{"lfm_contract":{"schema_version":1}}\n', encoding="utf-8")
    assert _non_lfm_fingerprint(paths, cfg) == before_non_lfm
    assert _lfm_fingerprint(paths, cfg) != before_lfm

    irodori = paths.models / "irodori" / "lora" / "checkpoint_final" / "adapter"
    seed_vc = paths.models / "seed_vc" / "cfm.pth"
    lfm = paths.models / "lfm" / "adapter" / "adapter"
    irodori.parent.mkdir(parents=True, exist_ok=True)
    irodori.write_bytes(b"irodori")
    seed_vc.parent.mkdir(parents=True, exist_ok=True)
    seed_vc.write_bytes(b"seed")
    lfm.parent.mkdir(parents=True, exist_ok=True)
    lfm.write_bytes(b"lfm")
    _invalidate_lfm_artifacts(paths)
    assert irodori.is_file() and seed_vc.is_file()
    assert not lfm.exists()


def test_prepare_accepts_legacy_lfm_rows_but_rejects_mixed_contract_markers(tmp_path: Path):
    legacy = tmp_path / "lfm_train.jsonl"
    legacy.write_text('{"prompt":[],"completion":[]}\n', encoding="utf-8")
    assert _jsonl_contract(legacy) == 1
    ...
    legacy.write_text(
        '{"lfm_contract":{"schema_version":1,"fingerprint":"wrong"}}\n',
        encoding="utf-8",
    )
    assert _jsonl_contract(legacy) is None
