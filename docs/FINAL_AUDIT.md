...
- ASR is intentionally fixed to `Systran/faster-whisper-large-v3@edaa852ec7e145841d8ffdb056a99866b5f0a478`. That revision remains the published `large-v3` CTranslate2 model revision used by the current repository. Runtime configuration cannot silently select a different remote ASR model.
- `pyannote/speaker-diarization-community-1@3533c8cf8e369892e6b79ff1bf80f7b0286a54ee` is loaded from the audited local snapshot and uses both regular and exclusive diarization plus speaker embeddings. The official model documentation explicitly supports loading the pipeline from a local directory for offline use.
- LFM runtime and fine-tuning both require `LiquidAI/LFM2.5-1.2B-JP-202606@b31023f2d69b95fbd7876898f8de9fae90e8afbd`; the local revision marker is verified before model loading. The model card's recommended generation parameters (`temperature=0.1`, `top_k=50`, `repetition_penalty=1.05`) are reflected by the worker defaults.
...
- Structural diarization failures remain errors. If the completed preparation contains zero usable authorized-speaker utterances, the prepare stage fails rather than publishing a false-success state.
...
- Prepare cache policy includes the audited ASR/diarization/Sense model contract plus the SHA256 of each isolated worker lockfile. Training fingerprints include the Irodori/LFM/Seed-VC source/model contract and relevant lockfile hashes. Dependency-graph changes therefore invalidate stale derived artifacts even when user input is unchanged.
