...
The checksum-hardening change was validated on GitHub Actions run #317 before the final documentation-only update: 12/12 jobs succeeded across Ubuntu/Windows core and all five isolated worker environments. Root jobs passed Ruff, the complete pytest suite, compileall, and CLI smoke tests. Regression coverage includes same-revision corrupt-weight rejection, failed-download marker suppression, setup re-materialization of corrupt snapshots, and root/worker hash-contract alignment.
