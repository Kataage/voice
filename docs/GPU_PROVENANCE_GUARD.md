...
- ASR setup preflight now loads every required Windows cuBLAS/cuDNN DLL (or the corresponding Linux shared libraries) and creates/destroys real cuBLAS and cuDNN handles before a CUDA compute type is accepted. A missing native runtime fails setup before model downloads or persona preparation.
...
- `cuda_preflight.py`, which implements the native execution gate, is fingerprinted by the environment contract. This change therefore makes existing setup state stale and requires `persona setup --backend auto` once after updating.
