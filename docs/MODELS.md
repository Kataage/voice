...
Training fingerprints include the pinned Irodori source revision, Irodori base/DACVAE hashes, ModernBERT revision, LFM base revision, Seed-VC source revision, relevant worker/managed lockfile hashes, and the training/Irodori/LFM-contract/Seed worker implementation hashes. The v0.3 train result also records independent non-LFM and LFM scope fingerprints: an explicit LFM export/config/contract change removes only LFM-derived adapter/checkpoints, while Irodori latents/checkpoints and Seed-VC artifacts remain available. Other training input changes and `--force` retain the existing full invalidation policy. If training artifacts exist without a recorded train-stage fingerprint, PersonaVoice treats them as untracked and rebuilds them instead of guessing their provenance.
