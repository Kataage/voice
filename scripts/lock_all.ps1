...
if (Test-Path (Join-Path $IrodoriDir "pyproject.toml")) {
    ...
    $Backup = if ($HadLock) { [System.IO.File]::ReadAllBytes((Resolve-Path $VendorLock)) } else { $null }

    try {
        uv lock --project $IrodoriDir
        Assert-NativeSuccess "uv lock (Irodori)"
        Copy-Item -Force $VendorLock $ManagedIrodoriLock
    }
    finally {
        if ($HadLock) {
            [System.IO.File]::WriteAllBytes((Join-Path (Get-Location) $VendorLock), $Backup)
        }
        else {
